...
export class TaxSummaryComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    ...
    if (this.itrObject) {
      ...
      this.itrObject = this.utilsService.createEmptyJson(
        this.userDetails,
        '2022-2023',
        '2021-2022'
      );
  ...
  parsJson(itrJsonInfo: any) {
    ...
    if (itrData.hasOwnProperty('ITR1')) {
      (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
        'itrType'
      ].setValue('1');
  ...
  setJsonData(itrData: any) {
    ...
    if (itrData.FilingStatus.ReturnFileSec === 17) {
      ...
      (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
        'ackNumber'
      ].setValue(itrData.FilingStatus.ReceiptNo);
      (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
        'eFillingDate'
      ].setValue(itrData.FilingStatus.OrigRetFiledDate);
    ...
    (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
      'aadharNumber'
    ].setValue(itrData.PersonalInfo.AadhaarCardNo);

    let natureOfEmployer = itrData.PersonalInfo.EmployerCategory;
    (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
      'employerCategory'
    ].setValue(natureOfEmployer);
    ...
    ).controls['premisesName'].setValue(mainAddress);
    (
      (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
        'address'
      ] as FormGroup
    ).controls['pinCode'].setValue(address.PinCode);
    this.getCityData(
      (
        (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
          'address'
    ...
    if (bankInfo instanceof Array && bankInfo.length > 0) {
      ...
      (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
        'bankDetails'
      ].setValue(this.bankData);
    }

    /* For itr-1 json there contain 'ITR1_IncomeDeductions' key and for itr-2 json cotain 'IncomeDeductions' key  */
    var incomeDeduction = itrData.hasOwnProperty('ITR1_IncomeDeductions')
      ? itrData.ITR1_IncomeDeductions
      : itrData.IncomeDeductions;

    /* House Property */
    var housingInfo = incomeDeduction;
    if (
      this.utilsService.isNonZero(housingInfo.GrossRentReceived) ||
      this.utilsService.isNonZero(housingInfo.AnnualValue) ||
      this.utilsService.isNonZero(housingInfo.TaxPaidlocalAuth) ||
      this.utilsService.isNonZero(housingInfo.InterestPayable) ||
      this.utilsService.isNonZero(housingInfo.TotalIncomeOfHP) ||
      this.utilsService.isNonZero(housingInfo.StandardDeduction)
    ) {
      var housingObj = {
        propertyType: housingInfo.hasOwnProperty('TypeOfHP')
          ? housingInfo.TypeOfHP === 'S'
            ? 'SOP'
            : 'LOP'
          : housingInfo.TotalIncomeOfHP === 0
          ? 'SOP'
          : 'LOP',
        address: '',
        ownerOfProperty: '',
        ...
        tenantName: '',
        tenentPanNumber: '',
        grossAnnualRentReceived: this.getNumberFormat(
          housingInfo.GrossRentReceived
        ),
    ...
    for (let i = 0; i < this.housingData.length; i++) {
      ...
      if (this.utilsService.isNonEmpty(this.housingData[i].interestAmount)) {
        let loanObj = {
          interestAmount: this.getNumberFormat(
            this.housingData[i].interestAmount
          ),
          loanType: 'HOUSING',
          principalAmount: 0,
        };
      ...
      if (
        this.utilsService.isNonEmpty(this.housingData[i].tenantName) &&
        this.utilsService.isNonEmpty(this.housingData[i].tenentPanNumber)
      ) {
        let tenantObj = {
    ...
    if (
      salaryInfo.hasOwnProperty('Salary') ||
      salaryInfo.hasOwnProperty('PerquisitesValue') ||
      salaryInfo.hasOwnProperty('ProfitsInSalary') ||
      this.utilsService.isNonZero(salaryInfo.GrossSalary) ||
      this.utilsService.isNonZero(
        salaryInfo.AllwncExemptUs10.TotalAllwncExemptUs10
      ) ||
      this.utilsService.isNonZero(salaryInfo.NetSalary) ||
      this.utilsService.isNonZero(salaryInfo.DeductionUs16ia) ||
      this.utilsService.isNonZero(salaryInfo.EntertainmentAlw16ii) ||
      this.utilsService.isNonZero(salaryInfo.ProfessionalTaxUs16iii) ||
      this.utilsService.isNonZero(salaryInfo.IncomeFromSal)
    ) {
      var salObj = {
        employerName: '',
        address: '',
        employerTAN: '',
        employerCategory: '',
        salAsPerSec171: salaryInfo.hasOwnProperty('Salary')
          ? this.getNumberFormat(salaryInfo.Salary)
          : 0,
        valOfPerquisites: salaryInfo.hasOwnProperty('PerquisitesValue')
          ? this.getNumberFormat(salaryInfo.PerquisitesValue)
          : 0,
        profitInLieu: salaryInfo.hasOwnProperty('ProfitsInSalary')
          ? this.getNumberFormat(salaryInfo.ProfitsInSalary)
          : 0,
        grossSalary: salaryInfo.GrossSalary,
        houseRentAllow: hra,
        leaveTravelExpense: 0,
        other: otherAmnt,
        totalExemptAllow: this.getNumberFormat(
          salaryInfo.AllwncExemptUs10.TotalAllwncExemptUs10
        ),
        ...
        professionalTax: this.getNumberFormat(
          salaryInfo.ProfessionalTaxUs16iii
        ),
        totalSalaryDeduction:
          Number(salaryInfo.DeductionUs16ia) +
          Number(salaryInfo.EntertainmentAlw16ii) +
    ...
    for (let i = 0; i < this.salaryItrratedData.length; i++) {
      ...
      if (
        this.utilsService.isNonEmpty(
          this.salaryItrratedData[i].houseRentAllow
        ) &&
        this.salaryItrratedData[i].houseRentAllow !== 0
      ) {
        let houceAllowObj = {
          allowanceType: 'HOUSE_RENT',
          description: null,
          exemptAmount: this.getNumberFormat(
            this.salaryItrratedData[i].houseRentAllow
          ),
          taxableAmount: 0,
        };
      ...
      // }
      if (
        this.utilsService.isNonEmpty(this.salaryItrratedData[i].other) &&
        this.salaryItrratedData[i].other !== 0
      ) {
        let otherAllowObj = {
      ...
      if (
        this.utilsService.isNonEmpty(
          this.salaryItrratedData[i].totalExemptAllow
        ) &&
        this.salaryItrratedData[i].totalExemptAllow !== 0
      ) {
      ...
      if (
        this.utilsService.isNonEmpty(
          this.salaryItrratedData[i].entertainAllow
        ) &&
        this.salaryItrratedData[i].entertainAllow !== 0
      ) {
      ...
      if (
        this.utilsService.isNonEmpty(
          this.salaryItrratedData[i].professionalTax
        ) &&
        this.salaryItrratedData[i].professionalTax !== 0
      ) {
      ...
      if (
        this.utilsService.isNonEmpty(
          this.salaryItrratedData[i].salAsPerSec171
        ) &&
        this.salaryItrratedData[i].salAsPerSec171 !== 0
      ) {
        let sal17Obj = {
          description: null,
          exemptAmount: 0,
          salaryType: 'SEC17_1',
          taxableAmount: this.getNumberFormat(
            this.salaryItrratedData[i].salAsPerSec171
          ),
        };
      ...
      if (
        this.utilsService.isNonEmpty(
          this.salaryItrratedData[i].valOfPerquisites
        ) &&
        this.salaryItrratedData[i].valOfPerquisites !== 0
      ) {
        let valOfPerqu17Obj = {
          description: null,
          exemptAmount: 0,
      ...
      if (
        this.utilsService.isNonEmpty(this.salaryItrratedData[i].profitInLieu) &&
        this.salaryItrratedData[i].profitInLieu !== 0
      ) {
        let profitsInLieuObj = {
          ...
          salaryType: 'SEC17_3',
          taxableAmount: this.getNumberFormat(
            this.salaryItrratedData[i].profitInLieu
          ),
        };
    ...
    if (incomeDeduction.hasOwnProperty('OthersInc')) {
      ...
      if (otherInfo instanceof Array && otherInfo.length > 0) {
        if (
          otherInfo.filter((item: any) => item.OthSrcNatureDesc === 'DIV')
            .length > 0
        ) {
          let dividentVal = otherInfo.filter(
            (item: any) => item.OthSrcNatureDesc === 'DIV'
          )[0].OthSrcOthAmount;
        ...
        if (
          otherInfo.filter((item: any) => item.OthSrcNatureDesc === 'SAV')
            .length > 0
        ) {
          let intOnSavingAcntVal = otherInfo.filter(
            (item: any) => item.OthSrcNatureDesc === 'SAV'
          )[0].OthSrcOthAmount;
        ...
        if (
          otherInfo.filter((item: any) => item.OthSrcNatureDesc === 'IFD')
            .length > 0
        ) {
          let intfromDepositeVal = otherInfo.filter(
            (item: any) => item.OthSrcNatureDesc === 'IFD'
          )[0].OthSrcOthAmount;
        ...
        if (
          otherInfo.filter((item: any) => item.OthSrcNatureDesc === 'TAX')
            .length > 0
        ) {
          let intFromIncoTaxVal = otherInfo.filter(
            (item: any) => item.OthSrcNatureDesc === 'TAX'
          )[0].OthSrcOthAmount;
          ...
          this.sourcesOfIncome.interestFromIncomeTax =
            Number(intFromIncoTaxVal);
        }
        if (
          otherInfo.filter((item: any) => item.OthSrcNatureDesc === 'FAP')
            .length > 0
        ) {
          let familyPension = otherInfo.filter(
            (item: any) => item.OthSrcNatureDesc === 'FAP'
          )[0].OthSrcOthAmount;
          ...
          this.sourcesOfIncome.familyPension =
            Number(familyPension) - Number(incomeDeduction.DeductionUs57iia);
        }
        if (
          otherInfo.filter((item: any) => item.OthSrcNatureDesc === 'OTH')
            .length > 0
        ) {
      ...
      this.sourcesOfIncome.interestFromOther =
        incomeDeduction.IncomeOthSrc -
        (this.sourcesOfIncome.dividend +
          this.sourcesOfIncome.interestFromSaving +
          this.sourcesOfIncome.interestFromBank +
          this.sourcesOfIncome.interestFromIncomeTax);

      this.sourcesOfIncome.toatlIncome =
        this.sourcesOfIncome.interestFromSaving +
        this.sourcesOfIncome.interestFromBank +
        this.sourcesOfIncome.interestFromIncomeTax +
        this.sourcesOfIncome.interestFromOther +
    ...
    var exemptIncData = itrData.hasOwnProperty('ITR1_IncomeDeductions')
      ? exemptIncomeInfo.ExemptIncAgriOthUs10Dtls
      : exemptIncomeInfo.OthersInc.OthersIncDtls;
    ...
    let totalExemptIncome = itrData.hasOwnProperty('ITR1_IncomeDeductions')
      ? exemptIncomeInfo.ExemptIncAgriOthUs10Total
      : exemptIncomeInfo.OthersInc.OthersTotalTaxExe;
    this.itrSummaryForm.controls['totalExemptIncome'].setValue(
      totalExemptIncome
    );
    ...
    this.sec80DobjVal.healthInsuarancePremiumSelf = this.getNumberFormat(
      sec80DInfo.Sec80DSelfFamSrCtznHealth.HealthInsPremSlfFam
    );
    this.sec80DobjVal.healthInsuarancePremiumParents =
      this.getNumberFormat(
        sec80DInfo.Sec80DSelfFamSrCtznHealth.ParentsSeniorCitizen
      ) -
      (sec80DInfo.Sec80DSelfFamSrCtznHealth.hasOwnProperty(
        ...
        : 0);
    var prehealthCheckVal = this.getNumberFormat(
      sec80DInfo.Sec80DSelfFamSrCtznHealth.hasOwnProperty(
    ...
    if (this.sec80DobjVal.parentAge === 'above60') {
      this.sec80DobjVal.medicalExpendature = this.getNumberFormat(
        sec80DInfo.Sec80DSelfFamSrCtznHealth.MedicalExpParentsSrCtzn
      );
    }
    ...
    if (sec80Ginfo.hasOwnProperty('Don100Percent')) {
      if (sec80Ginfo.Don100Percent.hasOwnProperty('DoneeWithPan')) {
        if (
          sec80Ginfo.Don100Percent.DoneeWithPan instanceof Array &&
          sec80Ginfo.Don100Percent.DoneeWithPan.length > 0
        ) {
          for (
            let i = 0;
    ...
    if (sec80Ginfo.hasOwnProperty('Don50PercentNoApprReqd')) {
      if (sec80Ginfo.Don50PercentNoApprReqd.hasOwnProperty('DoneeWithPan')) {
        if (
          sec80Ginfo.Don50PercentNoApprReqd.DoneeWithPan instanceof Array &&
          sec80Ginfo.Don50PercentNoApprReqd.DoneeWithPan.length > 0
        ) {
          for (
            let i = 0;
    ...
    if (sec80Ginfo.hasOwnProperty('Don100PercentApprReqd')) {
      if (sec80Ginfo.Don100PercentApprReqd.hasOwnProperty('DoneeWithPan')) {
        if (
          sec80Ginfo.Don100PercentApprReqd.DoneeWithPan instanceof Array &&
          sec80Ginfo.Don100PercentApprReqd.DoneeWithPan.length > 0
        ) {
          for (
            let i = 0;
    ...
    if (sec80Ginfo.hasOwnProperty('Don50PercentApprReqd')) {
      if (sec80Ginfo.Don50PercentApprReqd.hasOwnProperty('DoneeWithPan')) {
        if (
          sec80Ginfo.Don50PercentApprReqd.DoneeWithPan instanceof Array &&
          sec80Ginfo.Don50PercentApprReqd.DoneeWithPan.length > 0
        ) {
          for (
            let i = 0;
            ...
            let body = {
              ...
              amountInCash: this.getNumberFormat(
                sec80Ginfo.Don50PercentApprReqd.DoneeWithPan[i].DonationAmtCash
              ),
              amountOtherThanCash: this.getNumberFormat(
        ...
        if (scientificInfo instanceof Array && scientificInfo.length > 0) {
          for (let i = 0; i < scientificInfo.length; i++) {
            let body = {
              ...
              schemeCode: '',
              amountInCash: this.getNumberFormat(
                scientificInfo[i].DonationAmtCash
              ),
              amountOtherThanCash: this.getNumberFormat(
                scientificInfo[i].DonationAmtOtherMode
              ),
              eligibleAmount: this.getNumberFormat(
                scientificInfo[i].EligibleDonationAmt
              ),
              details: '',
              category: '',
            };
    ...
    if (itrData.hasOwnProperty('Schedule80GGC')) {
      ...
      if (
        politicalInfo.DonationDtlsSciRsrchRuralDev instanceof Array &&
        politicalInfo.DonationDtlsSciRsrchRuralDev.length > 0
      ) {
        for (
          let i = 0;
          i < politicalInfo.DonationDtlsSciRsrchRuralDev.length;
          i++
        ) {
          let body = {
            ...
            pinCode:
              sec80Ginfo.Don100Percent.DoneeWithPan[i]?.AddressDetail.PinCode,
            state:
              sec80Ginfo.Don100Percent.DoneeWithPan[i]?.AddressDetail.StateCode,
            panNumber: sec80Ginfo.Don100Percent.DoneeWithPan[i]?.DoneePAN,
            donationType: 'POLITICAL',
            schemeCode: '',
            amountInCash: this.getNumberFormat(
              politicalInfo.DonationDtlsSciRsrchRuralDev[i].DonationAmtCash
            ),
            amountOtherThanCash: this.getNumberFormat(
              politicalInfo.DonationDtlsSciRsrchRuralDev[i].DonationAmtOtherMode
            ),
            eligibleAmount: this.getNumberFormat(
              politicalInfo.DonationDtlsSciRsrchRuralDev[i].EligibleDonationAmt
            ),
            details: '',
            category: '',
          };
    ...
    this.itrSummaryForm.controls['us80ccc'].setValue(
      deductionValues.Section80CCC
    );
    this.itrSummaryForm.controls['us80ccc1'].setValue(
      deductionValues.Section80CCDEmployeeOrSE
    );
    this.itrSummaryForm.controls['us80ccd2'].setValue(
      deductionValues.Section80CCDEmployer
    );
    this.itrSummaryForm.controls['us80ccd1b'].setValue(
      deductionValues.Section80CCD1B
    );
    this.itrSummaryForm.controls['us80dd'].setValue(
      deductionValues.Section80DD
    );
    this.itrSummaryForm.controls['us80ddb'].setValue(
      deductionValues.Section80DDB
    );
    this.itrSummaryForm.controls['us80e'].setValue(deductionValues.Section80E);
    this.itrSummaryForm.controls['us80ee'].setValue(
      deductionValues.Section80EE
    );
    this.itrSummaryForm.controls['us80gg'].setValue(
      deductionValues.Section80GG
    );
    this.itrSummaryForm.controls['us80gga'].setValue(
      deductionValues.Section80GGA
    );
    this.itrSummaryForm.controls['us80ggc'].setValue(
      deductionValues.Section80GGC
    );
    this.itrSummaryForm.controls['us80ttaTtb'].setValue(
    ...
    this.itrSummaryForm.controls['us80eeb'].setValue(
      deductionValues.Section80EEB
    );
    this.itrSummaryForm.controls['us80eea'].setValue(
    ...
    if (this.newTaxRegime) {
      this.newRegimeTaxesPaid.tdsOnSalary =
        itrData.TDSonSalaries.TotalTDSonSalaries;
    } else {
    ...
    if (tdsOnSalInfo instanceof Array && tdsOnSalInfo.length > 0) {
      for (let i = 0; i < tdsOnSalInfo.length; i++) {
        ...
        tdsObj.deductorTAN =
          tdsOnSalInfo[i].EmployerOrDeductorOrCollectDetl.TAN;
        tdsObj.deductorName =
          tdsOnSalInfo[
            i
          ].EmployerOrDeductorOrCollectDetl.EmployerOrDeductorOrCollecterName;
        tdsObj.totalAmountCredited = this.getNumberFormat(
          tdsOnSalInfo[i].IncChrgSal
        );
        tdsObj.totalTdsDeposited = this.getNumberFormat(
    ...
    if (itrData?.TDSonOthThanSals?.hasOwnProperty('TDSonOthThanSal')) {
      ...
      itrData?.TDSonOthThanSals?.hasOwnProperty('TDSonOthThanSalDtls')
    ) {
    ...
    // }
    if (
      tdsOtherThanSalInfo instanceof Array &&
      tdsOtherThanSalInfo.length > 0
    ) {
      for (let i = 0; i < tdsOtherThanSalInfo.length; i++) {
        ...
        tdsOtherThanSalObj.deductorTAN = this.itrType.itrOne
          ? tdsOtherThanSalInfo[i].EmployerOrDeductorOrCollectDetl.TAN
          : tdsOtherThanSalInfo[i].TANOfDeductor;
        tdsOtherThanSalObj.deductorName = this.itrType.itrOne
          ? tdsOtherThanSalInfo[i].EmployerOrDeductorOrCollectDetl
              .EmployerOrDeductorOrCollecterName
          : tdsOtherThanSalInfo[i].HeadOfIncome;
        tdsOtherThanSalObj.totalAmountCredited = this.itrType.itrOne
          ? this.getNumberFormat(tdsOtherThanSalInfo[i].AmtForTaxDeduct)
          : this.getNumberFormat(tdsOtherThanSalInfo[i].GrossAmount);
        tdsOtherThanSalObj.totalTdsDeposited = this.itrType.itrOne
          ? this.getNumberFormat(tdsOtherThanSalInfo[i].TotTDSOnAmtPaid)
          : this.getNumberFormat(tdsOtherThanSalInfo[i].TDSDeducted);
    ...
    if (this.newTaxRegime) {
      this.newRegimeTaxesPaid.tdsOtherThanSalary =
        Number(itrData?.TDSonOthThanSals?.TotalTDSonOthThanSals) +
        (itrData?.ScheduleTDS3Dtls?.TotalTDS3Details
          ? Number(itrData?.ScheduleTDS3Dtls?.TotalTDS3Details)
          : 0);
    } else {
    ...
    if (
      tds3OtherThanSalInfo instanceof Array &&
      tds3OtherThanSalInfo.length > 0
    ) {
      for (let i = 0; i < tds3OtherThanSalInfo.length; i++) {
        ...
        tdsOtherThanSalObj.deductorName = this.itrType.itrOne
          ? tds3OtherThanSalInfo[i].NameOfTenant
          : tds3OtherThanSalInfo[i].HeadOfIncome;
        tdsOtherThanSalObj.totalAmountCredited = this.itrType.itrOne
          ? this.getNumberFormat(tds3OtherThanSalInfo[i].GrsRcptToTaxDeduct)
          : this.getNumberFormat(tds3OtherThanSalInfo[i].GrossAmount);
        tdsOtherThanSalObj.totalTdsDeposited = this.getNumberFormat(
          tds3OtherThanSalInfo[i].TDSDeducted
        );
        this.tdsOtherThanSal.push(tdsOtherThanSalObj);
        this.taxPaiObj.otherThanSalary16A.push(tdsOtherThanSalObj);
      }
    ...
    if (tcsInfo instanceof Array && tcsInfo.length > 0) {
      for (let i = 0; i < tcsInfo.length; i++) {
        ...
        tcsObj.collectorName =
          tcsInfo[
            i
          ].EmployerOrDeductorOrCollectDetl.EmployerOrDeductorOrCollecterName;
        tcsObj.totalAmountPaid = this.itrType.itrOne
          ? this.getNumberFormat(tcsInfo[i].AmtTaxCollected)
          : this.getNumberFormat(tcsInfo[i].Amtfrom26AS);
    ...
    var advanceTaxInfo = itrData.hasOwnProperty('ITR1_IncomeDeductions')
      ? itrData.TaxPayments.TaxPayment
      : itrData.ScheduleIT.TaxPayment;
    this.advanceSelfTax = [];
    if (this.newTaxRegime) {
      this.newRegimeTaxesPaid.advanceSelfAssTax = itrData.hasOwnProperty(
        'ITR1_IncomeDeductions'
      )
        ? itrData.TaxPayments.TotalTaxPayments
        : itrData.ScheduleIT.TotalTaxPayments;
    } else {
      this.taxesPaid.advanceSelfAssTax = itrData.hasOwnProperty(
        'ITR1_IncomeDeductions'
      )
        ? itrData.TaxPayments.TotalTaxPayments
        : itrData.ScheduleIT.TotalTaxPayments;
    ...
    (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
      'taxPaid'
    ].setValue(this.taxPaiObj);

    var totalTaxPaidVal;
    if (this.newTaxRegime) {
      this.newRegimeTaxSummary.totalTaxesPaid =
        Number(this.newRegimeTaxesPaid.tdsOnSalary) +
        Number(this.newRegimeTaxesPaid.tdsOtherThanSalary) +
        Number(this.newRegimeTaxesPaid.tdsOnSal26QB) +
        Number(this.newRegimeTaxesPaid.tcs) +
        Number(this.newRegimeTaxesPaid.advanceSelfAssTax);

      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'totalTaxesPaid'
      ].setValue(this.newRegimeTaxSummary.totalTaxesPaid);
    } else {
      totalTaxPaidVal =
    ...
    if (this.newTaxRegime) {
      ...
      this.newRegimeTaxSummary.housePropertyIncome =
        computation1Info.TotalIncomeOfHP;

      let totalOtherIncomVal =
        computation1Info.IncomeOthSrc + computation1Info.DeductionUs57iia;
      ...
      this.newRegimeTaxSummary.totalDeduction =
        incomeDeduction.DeductUndChapVIA.TotalChapVIADeductions;

      this.newRegimeTaxSummary.totalIncomeAfterDeductionIncludeSR =
        this.getNumberFormat(this.newRegimeTaxSummary.grossTotalIncome) -
        this.getNumberFormat(this.newRegimeTaxSummary.totalDeduction);

      let computation2Infos = itrData.ITR1_TaxComputation;
      this.newRegimeTaxSummary.taxOnTotalIncome = this.getNumberFormat(
        computation2Infos.TotalTaxPayable
      );
      this.newRegimeTaxSummary.forRebate87Tax = this.getNumberFormat(
        computation2Infos.Rebate87A
      );
      this.newRegimeTaxSummary.taxAfterRebate = this.getNumberFormat(
        computation2Infos.TaxPayableOnRebate
      );
      this.newRegimeTaxSummary.cessAmount = this.getNumberFormat(
        computation2Infos.EducationCess
      );
      this.newRegimeTaxSummary.totalTax = this.getNumberFormat(
        computation2Infos.GrossTaxLiability
      );
      this.newRegimeTaxSummary.taxReliefUnder89 = this.getNumberFormat(
        computation2Infos.Section89
      );
      this.newRegimeTaxSummary.totalTaxRelief = this.getNumberFormat(
        computation2Infos.NetTaxLiability
      );
      this.newRegimeTaxSummary.s234A = this.getNumberFormat(
        computation2Infos.IntrstPay.IntrstPayUs234A
      );
      this.newRegimeTaxSummary.s234B = this.getNumberFormat(
        computation2Infos.IntrstPay.IntrstPayUs234B
      );
      this.newRegimeTaxSummary.s234C = this.getNumberFormat(
        computation2Infos.IntrstPay.IntrstPayUs234C
      );
      this.newRegimeTaxSummary.s234F = this.getNumberFormat(
        computation2Infos.IntrstPay.LateFilingFee234F
      );
      this.newRegimeTaxSummary.agrigateLiability = this.getNumberFormat(
        computation2Infos.TotalIntrstPay
      );
      ...
      if (this.getNumberFormat(taxesPaidInfos.TaxPaid.BalTaxPayable) > 0) {
        let payable = this.getNumberFormat(
          taxesPaidInfos.TaxPaid.BalTaxPayable
        );
      ...
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        's234A'
      ].setValue(this.newRegimeTaxSummary.s234A);
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        's234B'
      ].setValue(this.newRegimeTaxSummary.s234B);
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        's234C'
      ].setValue(this.newRegimeTaxSummary.s234C);
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        's234F'
      ].setValue(this.newRegimeTaxSummary.s234F);

      //Old summary part

      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'salary'
      ].setValue(computation1Info.IncomeFromSal);
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'housePropertyIncome'
      ].setValue(computation1Info.TotalIncomeOfHP);

      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'otherIncome'
      ].setValue(computation1Info.IncomeOthSrc);
      ...
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'grossTotalIncome'
      ].setValue(grossTotIncome);

      let dedUnderChaVIA = this.getNumberFormat(
        incomeDeduction.DeductUndChapVIA.TotalChapVIADeductions
      );
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'totalDeduction'
      ].setValue(dedUnderChaVIA);

      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'totalIncomeAfterDeductionIncludeSR'
      ].setValue(computation1Info.TotalIncome);
      ...
      let computation2Info = itrData.hasOwnProperty('ITR1_IncomeDeductions')
        ? itrData.ITR1_TaxComputation
        : itrData.TaxComputation;
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'taxOnTotalIncome'
      ].setValue(this.getNumberFormat(computation2Info.TotalTaxPayable));
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'forRebate87Tax'
      ].setValue(this.getNumberFormat(computation2Info.Rebate87A));

      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'taxAfterRebate'
      ].setValue(this.getNumberFormat(computation2Info.TaxPayableOnRebate));
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'cessAmount'
      ].setValue(this.getNumberFormat(computation2Info.EducationCess));
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'totalTax'
      ].setValue(this.getNumberFormat(computation2Info.GrossTaxLiability));
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'taxReliefUnder89'
      ].setValue(this.getNumberFormat(computation2Info.Section89));
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'totalTaxRelief'
      ].setValue(this.getNumberFormat(computation2Info.NetTaxLiability));
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        's234A'
      ].setValue(
        this.getNumberFormat(computation2Info.IntrstPay.IntrstPayUs234A)
      );
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        's234B'
      ].setValue(
        this.getNumberFormat(computation2Info.IntrstPay.IntrstPayUs234B)
      );
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        's234C'
      ].setValue(
        this.getNumberFormat(computation2Info.IntrstPay.IntrstPayUs234C)
      );
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        's234F'
      ].setValue(
        this.getNumberFormat(computation2Info.IntrstPay.LateFilingFee234F)
      );
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'agrigateLiability'
      ].setValue(this.getNumberFormat(computation2Info.TotalIntrstPay));

      let taxesPaidInfo = itrData;

      this.taxesPaid.tdsOnSalary = taxesPaidInfo.hasOwnProperty('TDSonSalaries')
        ? taxesPaidInfo.TDSonSalaries.TotalTDSonSalaries
        : 0;
      this.taxesPaid.tdsOtherThanSalary = taxesPaidInfo.hasOwnProperty(
        'TDSonOthThanSals'
      )
        ? Number(itrData.TDSonOthThanSals.TotalTDSonOthThanSals) +
          (itrData?.ScheduleTDS3Dtls?.TotalTDS3Details
      ...
      this.taxesPaid.tcs = taxesPaidInfo.hasOwnProperty('ScheduleTCS')
        ? taxesPaidInfo?.ScheduleTCS?.TotalSchTCS
        : 0;
      ...
      this.taxesPaid.advanceSelfAssTax = taxesPaidInfo.hasOwnProperty('TaxPaid')
        ? taxesPaidInfo.TaxPaid.TaxesPaid.SelfAssessmentTax
        : 0;
  ...
  setTaxRegimeValue(filingStatus) {
    ...
    (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
      'regime'
    ].setValue(filingStatus.NewTaxRegime);
  }

  setAnyotherDeductionValue(deductionValues) {
    let total =
      Number(this.itrSummaryForm.controls['us80c'].value) +
    ...
    this.itrSummaryForm.controls['other'].setValue(
  ...
  businessIncomeBind(itrData: any) {
    ...
    if (pre44ADinfo.hasOwnProperty('PersumptiveInc44AD')) {
      let recivedInBankObj = {
        ...
        minimumPresumptiveIncome: pre44ADinfo.PersumptiveInc44AD.hasOwnProperty(
          ...
          ? this.getNumberFormat(
              pre44ADinfo.PersumptiveInc44AD.PersumptiveInc44AD6Per
            )
          : 0,
        ownership: null,
        periodOfHolding: 0,
        ...
        receipts: pre44ADinfo.PersumptiveInc44AD.hasOwnProperty(
          ...
          ? this.getNumberFormat(pre44ADinfo.PersumptiveInc44AD.GrsTrnOverBank)
          : 0,
        registrationNo: null,
        tonnageCapacity: 0,
      };
      ...
      let recivedCashObj = {
        ...
        minimumPresumptiveIncome: pre44ADinfo.PersumptiveInc44AD.hasOwnProperty(
          ...
          ? this.getNumberFormat(
              pre44ADinfo.PersumptiveInc44AD.PersumptiveInc44AD8Per
            )
          : 0,
        ownership: null,
        periodOfHolding: 0,
        ...
        receipts: pre44ADinfo.PersumptiveInc44AD.hasOwnProperty(
          ...
          ? this.getNumberFormat(
              pre44ADinfo.PersumptiveInc44AD.GrsTrnOverAnyOthMode
            )
          : 0,
        registrationNo: null,
        tonnageCapacity: 0,
      ...
      let grossRecipt44ADAObj = {
        ...
        minimumPresumptiveIncome:
          pre44ADAinfo.PersumptiveInc44ADA.hasOwnProperty(
            ...
            ? this.getNumberFormat(
                pre44ADAinfo.PersumptiveInc44ADA.TotPersumptiveInc44ADA
              )
            : 0,
        ownership: null,
        periodOfHolding: 0,
        presumptiveIncome: pre44ADAinfo.PersumptiveInc44ADA.hasOwnProperty(
          'TotPersumptiveInc44ADA'
        )
          ? this.getNumberFormat(
              pre44ADAinfo.PersumptiveInc44ADA.TotPersumptiveInc44ADA
          ...
          : 0,
        receipts: pre44ADAinfo.PersumptiveInc44ADA.hasOwnProperty('GrsReceipt')
          ? this.getNumberFormat(pre44ADAinfo.PersumptiveInc44ADA.GrsReceipt)
          : 0,
        registrationNo: null,
        tonnageCapacity: 0,
      };
    ...
    itr4Summary.assesse.business.financialParticulars.membersOwnCapital =
      this.getNumberFormat(financialInfo.PartnerMemberOwnCapital);
    itr4Summary.assesse.business.financialParticulars.securedLoans =
      this.getNumberFormat(financialInfo.SecuredLoans);
    itr4Summary.assesse.business.financialParticulars.unSecuredLoans =
      this.getNumberFormat(financialInfo.UnSecuredLoans);
    itr4Summary.assesse.business.financialParticulars.advances =
      this.getNumberFormat(financialInfo.Advances);
    itr4Summary.assesse.business.financialParticulars.sundryCreditorsAmount =
      this.getNumberFormat(financialInfo.SundryCreditors);
    itr4Summary.assesse.business.financialParticulars.otherLiabilities =
      this.getNumberFormat(financialInfo.OthrCurrLiab);
    let liabilityTotal =
      itr4Summary.assesse.business.financialParticulars.membersOwnCapital +
      itr4Summary.assesse.business.financialParticulars.securedLoans +
      itr4Summary.assesse.business.financialParticulars.unSecuredLoans +
    ...
    itr4Summary.assesse.business.financialParticulars.totalCapitalLiabilities =
      this.getNumberFormat(financialInfo.TotCapLiabilities);

    //Assets
    itr4Summary.assesse.business.financialParticulars.fixedAssets =
      this.getNumberFormat(financialInfo.FixedAssets);
    itr4Summary.assesse.business.financialParticulars.inventories =
      this.getNumberFormat(financialInfo.Inventories);
    itr4Summary.assesse.business.financialParticulars.sundryDebtorsAmount =
      this.getNumberFormat(financialInfo.SundryDebtors);
    itr4Summary.assesse.business.financialParticulars.balanceWithBank =
      this.getNumberFormat(financialInfo.BalWithBanks);
    itr4Summary.assesse.business.financialParticulars.cashInHand =
      this.getNumberFormat(financialInfo.CashInHand);
    itr4Summary.assesse.business.financialParticulars.loanAndAdvances =
      this.getNumberFormat(financialInfo.LoansAndAdvances);
    itr4Summary.assesse.business.financialParticulars.otherAssets =
      this.getNumberFormat(financialInfo.OtherAssets);
    let assetsTotal =
      itr4Summary.assesse.business.financialParticulars.fixedAssets +
      itr4Summary.assesse.business.financialParticulars.inventories +
      itr4Summary.assesse.business.financialParticulars.sundryDebtorsAmount +
    ...
    itr4Summary.assesse.business.financialParticulars.totalAssets =
      this.getNumberFormat(financialInfo.TotalAssets);
  ...
  addExemptIncome() {
    if (
      this.utilsService.isNonEmpty(this.exemptInfo.type) &&
      this.utilsService.isNonEmpty(this.exemptInfo.amount)
    ) {
  ...
  setSummary(summary) {
    if (summary.assesse.itrType === '1' || summary.assesse.itrType === '4') {
      ...
      if (
        ...
        var businessIncome = summary.assesse.business.presumptiveIncomes.filter(
          (item: any) => item.businessType === 'BUSINESS'
        );
        if (businessIncome.length > 0) {
          let businessNatureCode = businessIncome[0].natureOfBusiness;
          let recivedInBank = businessIncome[0].incomes.filter(
            (item: any) => item.incomeType === 'BANK'
          );
          let recivedInCash = businessIncome[0].incomes.filter(
            (item: any) => item.incomeType === 'CASH'
          );

          this.businessObject.tradeName44AD = businessIncome[0].tradeName;
          this.businessObject.received44ADtaotal =
            this.getNumberFormat(recivedInBank[0]?.receipt) +
            this.getNumberFormat(recivedInCash[0]?.receipts);
          this.businessObject.presumptive44ADtotal =
            this.getNumberFormat(recivedInBank[0]?.presumptiveIncome) +
            this.getNumberFormat(recivedInCash[0]?.presumptiveIncome);
        }

        var presumptiveIncome =
          summary.assesse.business.presumptiveIncomes.filter(
        ...
        if (presumptiveIncome.length > 0) {
          ...
          this.businessObject.grossReciept =
            presumptiveIncome[0].incomes[0]?.receipts;
          this.businessObject.presumptiveIncome =
            presumptiveIncome[0].incomes[0]?.presumptiveIncome;
        }
      }
      ...
      this.bankData = (
        this.itrSummaryForm.controls['assesse'] as FormGroup
      ).controls['bankDetails'].value;
      this.housingData = (
        this.itrSummaryForm.controls['assesse'] as FormGroup
      ).controls['houseProperties'].value;
      //this.salaryItrratedData = this.itrSummaryForm.controls['assesse'] as FormGroup).controls['employers.value;
      this.updateSalatyInfo(
        (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
          'employers'
        ]
      );
      ...
      this.donationData = (
        this.itrSummaryForm.controls['assesse'] as FormGroup
      ).controls['donations'].value;
      this.updateTaxDeductionAtSourceVal(
        (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
          'taxPaid'
        ]
      );
  ...
  updateInuranceVal(insuranceInfo: any) {
    ...
    if (insuranceInfo.value) {
      ...
      for (let i = 0; i < insuranceVal.length; i++) {
        if (
          ...
          this.utilsService.isNonEmpty(insuranceVal[i].premium)
        ) {
          this.sec80DobjVal.healthInsuarancePremiumSelf =
            insuranceVal[i].premium;
        }
        if (
          insuranceVal[i].policyFor === 'DEPENDANT' &&
          this.utilsService.isNonEmpty(insuranceVal[i].preventiveCheckUp)
        ) {
          this.sec80DobjVal.preventiveHealthCheckupFamily =
    ...
    if (
      ...
      ).controls['hasParentOverSixty'].value
    ) {
  ...
  getCodeFromLabelOnBlur() {
    if (
      this.utilsService.isNonEmpty(this.natureOfBusinessForm.value) &&
      this.utilsService.isNonEmpty(this.natureOfBusinessForm.value)
    ) {
      this.natureCode = this.natureOfBusinessDropdown44AD.filter(
        (item: any) =>
          item.label.toLowerCase() ===
          this.natureOfBusinessForm.value.toLowerCase()
      );
  ...
  openDialog(
    ...
    let disposable = this.dialog.open(SumaryDialogComponent, {
      width:
        mode === 'Salary' || mode === 'donationSec80G' || mode === 'House'
          ? '70%'
          : '30%',
      height: 'auto',
      data: {
    ...
    disposable.afterClosed().subscribe((result) => {
      ...
      if (result) {
        if (result.data.type === 'Bank') {
          ...
          this.setTdsOnSalValue(
            result.data.onSalary,
            result.data.action,
            result.data.index
          );
        } else if (result.data.type === 'tdsOnOtherThanSal') {
          this.setTdsOnOtherThanSalValue(
            result.data.otherThanSalary16A,
            result.data.action,
            result.data.index
          );
        } else if (result.data.type === 'tdsOnSalOfPro26Q') {
          this.setTdsOnSal26QValue(
            result.data.otherThanSalary26QB,
            result.data.action,
            result.data.index
          );
          ...
          this.setAdvanSelfAssTaxValue(
            result.data.otherThanTDSTCS,
            result.data.action,
            result.data.index
          );
        }
    ...
    if (action === 'Add') {
      if (this.bankData.length !== 0) {
        ...
        this.bankData
          ...
          .controls['bankDetails'].setValue(this.bankData);
      }
    } else if (action === 'Edit') {
      if (latestBankInfo.hasRefund === true) {
        for (let i = 0; i < this.bankData.length; i++) {
  ...
  setHousingData(housingData: any, action: any, index: any) {
    if (action === 'Add') {
      ...
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'housePropertyIncome'
      ].setValue(totalTaxableIncome); //Here we add total of taxableIncome into housePropertyIncome to show in table 2nd point
  ...
  createHouseDataObj(houseData: any, action: any, index: any) {
    if (action === 'Add') {
      let flatNo = this.utilsService.isNonEmpty(houseData[0].flatNo)
        ? houseData[0].flatNo
        : '';
      let building = this.utilsService.isNonEmpty(houseData[0].building)
        ? houseData[0].building
        : '';
      let street = this.utilsService.isNonEmpty(houseData[0].street)
        ? houseData[0].street
        : '';
      let locality = this.utilsService.isNonEmpty(houseData[0].locality)
        ? houseData[0].locality
        : '';
      let city = this.utilsService.isNonEmpty(houseData[0].city)
        ? houseData[0].city
        : '';
      let country = this.utilsService.isNonEmpty(houseData[0].country)
        ? houseData[0].country
        : '';
      let state = this.utilsService.isNonEmpty(houseData[0].state)
        ? houseData[0].state
        : '';
      let address =
        flatNo +
      ...
      for (let i = 0; i < houseData.length; i++) {
        let house = {
          ...
          pinCode: this.utilsService.isNonEmpty(houseData[i].pinCode)
            ? houseData[i].pinCode
            : '',
          flatNo: this.utilsService.isNonEmpty(houseData[i].flatNo)
            ? houseData[i].flatNo
            : '',
          building: this.utilsService.isNonEmpty(houseData[i].building)
            ? houseData[i].building
            : '',
          street: this.utilsService.isNonEmpty(houseData[i].street)
            ? houseData[i].street
            : '',
          locality: this.utilsService.isNonEmpty(houseData[i].locality)
            ? houseData[i].locality
            : '',
          city: this.utilsService.isNonEmpty(houseData[i].city)
            ? houseData[i].city
            : '',
          country: this.utilsService.isNonEmpty(houseData[i].country)
            ? houseData[i].country
            : '',
          state: this.utilsService.isNonEmpty(houseData[i].state)
            ? houseData[i].state
            : '',
        };
      ...
      let house = {
        ...
        tenantName:
          Array.isArray(houseData[0].tenant) && houseData[0].tenant.length > 0
            ? houseData[0].tenant[0].name
            : '',
        tenentPanNumber:
          Array.isArray(houseData[0].tenant) && houseData[0].tenant.length > 0
            ? houseData[0].tenant[0].panNumber
            : '',
        grossAnnualRentReceived: houseData[0].grossAnnualRentReceived
          ? houseData[0].grossAnnualRentReceived
          : '',
        annualValue: houseData[0].annualValue,
        propertyTax: houseData[0].propertyTax,
        interestAmount:
          Array.isArray(houseData[0].loans) && houseData[0].loans.length > 0
            ? houseData[0].loans[0].interestAmount
            : '',
        taxableIncome: houseData[0].taxableIncome,
        exemptIncome: houseData[0].exemptIncome,
        pinCode: this.utilsService.isNonEmpty(houseData[0].pinCode)
          ? houseData[0].pinCode
          : '',
        flatNo: this.utilsService.isNonEmpty(houseData[0].flatNo)
          ? houseData[0].flatNo
          : '',
        building: this.utilsService.isNonEmpty(houseData[0].building)
          ? houseData[0].building
          : '',
        street: this.utilsService.isNonEmpty(houseData[0].street)
          ? houseData[0].street
          : '',
        locality: this.utilsService.isNonEmpty(houseData[0].locality)
          ? houseData[0].locality
          : '',
        city: this.utilsService.isNonEmpty(houseData[0].city)
          ? houseData[0].city
          : '',
        country: this.utilsService.isNonEmpty(houseData[0].country)
          ? houseData[0].country
          : '',
        state: this.utilsService.isNonEmpty(houseData[0].state)
          ? houseData[0].state
          : '',
      };
      ...
      if (pincode.valid) {
        ...
        this.userService.getMethod(param).subscribe(
          (result: any) => {
            ...
            ).controls['country'].setValue('INDIA');
            (
              (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
                'address'
              ] as FormGroup
            ...
            ).controls['state'].setValue(result.stateName);
        ...
        (result: any) => {
          ...
          if (this.utilsService.isNonEmpty(dob)) {
            ...
            if (!this.utilsService.isNonEmpty(result.middleName)) {
              reqBody.fathersName =
                personalInfo.Verification.Declaration.FatherName;
  ...
  setProfileAddressValToHouse() {
    (this.itrSummaryForm.controls['houseProperties'] as FormGroup).controls[
      'locality'
    ].setValue(this.itrSummaryForm.controls['address'].value);
    (this.itrSummaryForm.controls['houseProperties'] as FormGroup).controls[
      'country'
    ].setValue('INDIA'); //91
    (this.itrSummaryForm.controls['houseProperties'] as FormGroup).controls[
      'pinCode'
    ].setValue(this.itrSummaryForm.controls['pinCode'].value);
    (this.itrSummaryForm.controls['houseProperties'] as FormGroup).controls[
      'city'
    ].setValue(this.itrSummaryForm.controls['city'].value);
    (this.itrSummaryForm.controls['houseProperties'] as FormGroup).controls[
      'state'
    ].setValue(this.itrSummaryForm.controls['state'].value);
  }
  ...
  setOtherSourceIncomeValue(incomeVal: any, type: any) {
    if (Number(incomeVal) !== 0 && this.utilsService.isNonEmpty(incomeVal)) {
      ...
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'otherIncome'
      ].setValue(this.sourcesOfIncome.toatlIncome);
      this.calculateGrossTotalIncome();
    } else {
      if (
        Number(incomeVal) === 0 ||
  ...
  setAnnualVal() {
    ...
    if (
      ...
      let standerdDeduct = Math.round(
        (Number(
          (this.itrSummaryForm.controls['houseProperties'] as FormGroup)
            .controls['annualValue'].value
        ) *
          30) /
          100
      );
  ...
  setNetHousingProLoan() {
    this.mainHousingData.push(
      this.itrSummaryForm.controls['houseProperties'].value[0]
    );
    ...
    let netHousingProLoan =
      Number(
        (this.itrSummaryForm.controls['houseProperties'] as FormGroup).controls[
          'annualValue'
        ].value
      ) - Number(this.itrSummaryForm.controls['hpStandardDeduction'].value);
    var netHousing = [];
  ...
  setTotalOfExempt() {
    this.totalOfExcempt =
      Number(this.itrSummaryForm.controls['ppfInterest'].value) +
      Number(this.itrSummaryForm.controls['giftFromRelative'].value) +
      Number(this.itrSummaryForm.controls['anyOtherExcemptIncome'].value);
  }
  ...
  setDeduction80DVal(insuranceVal: any, type: any) {
    ...
    if (insuranceVal !== 0 || this.utilsService.isNonEmpty(insuranceVal)) {
      ...
      let sec80dVal =
        Number(this.sec80DobjVal.healthInsuarancePremiumSelf) +
        Number(this.sec80DobjVal.healthInsuarancePremiumParents) +
        Number(this.sec80DobjVal.preventiveHealthCheckupFamily) +
        Number(this.sec80DobjVal.medicalExpendature);
  ...
  calculateRebateus87A() {
    //Calculate point 8 (Less: Rebate u/s 87A)
    //
  ...
  calculateTaxAfterRebate() {
    //Calculate point 9 (Tax after rebate (7-8))
    let taxAfterRebat =
      Number(
        (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
          'taxOnTotalIncome'
  ...
  calculateHealthEducsCess() {
    //Calculate point 10 (Tax after rebate (7-8))
    if (
      (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
        'taxAfterRebate'
      ].value > 0
  ...
  calculateTotalTaxCess() {
    //Calculate point 11 (Total tax & cess (9 + 10))
  ...
  calculateTaxAfterRelif() {
    //Calculate point 13 (Tax after relief  (11-12))
    let taxAfterRelif =
      Number(
        (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
          'totalTax'
  ...
  calculateTotalInterestFees() {
    //Calculate point 14 & 15 (Total Tax, fee and Interest (13+14))
    this.totalInterest =
      Number(
        (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
          's234A'
    ...
    ].setValue(this.totalInterest);

    let totalInterstFees =
      Number(
        (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
          'totalTaxRelief'
        ].value
    ...
    (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
      'agrigateLiability'
    ].setValue(totalInterstFees); //Point 15

    this.calculateNetTaxPayble();
  }

  calculateNetTaxPayble() {
    //Calculate point 17 (Net Tax Payable/ (Refund) (15 - 16))
    // alert('call...')
    let netTaxPayble =
      Number(
        (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
  ...
  setValueInLoanObj(interestAmount: any) {
    if (interestAmount.valid) {
      ...
      ).controls['loanType'].setValue('HOUSING');
    } else {
      (
        (this.itrSummaryForm.controls['houseProperties'] as FormGroup).controls[
          'loans'
        ] as FormGroup
      ).controls['loanType'].setValue(null);
    }
  }
  ...
  saveItrSummary() {
    ...
    if (
      this.itrSummaryForm.valid &&
      (this.itrType.itrFour ? this.businessFormValid : true)
    ) {
      if (this.utilsService.isNonEmpty(this.sourcesOfIncome)) {
        ...
        if (this.sourcesOfIncome.dividend !== 0) {
          ...
          let dividends = this.itrObject.dividendIncomes.filter(
            (item: any) => item.quarter != 1
          );
          this.itrObject.dividendIncomes = [];
          this.itrObject.dividendIncomes.push(dividendObj);
      ...
      this.itrSummaryForm.controls['newTaxRegime'].setValue(
      ...
      if (this.utilsService.isNonEmpty(this.sec80DobjVal)) {
        ...
        if (
          ...
          let obj = {
            ...
            typeOfPolicy: '',
            policyFor: 'DEPENDANT',
            premium: this.sec80DobjVal.healthInsuarancePremiumSelf
              ? this.sec80DobjVal.healthInsuarancePremiumSelf
              : 0,
            sumAssured: 0,
            healthCover: null,
        ...
        if (
          this.sec80DobjVal.healthInsuarancePremiumParents !== 0 ||
          this.sec80DobjVal.medicalExpendature !== 0
        ) {
          let obj = {
            insuranceType: 'HEALTH',
            typeOfPolicy: '',
            ...
            details: '',
            preventiveCheckUp: 0,
            medicalExpenditure: this.newItrSumChanges
              ? this.sec80DobjVal.parentAge === 'above60'
                ? this.sec80DobjVal.medicalExpendature
                : 0
              : 0,
          };
          insuranceData.push(obj);
        ...
        if (
          this.sec80DobjVal.parentAge !== '' ||
          this.sec80DobjVal.parentAge !== null
        ) {
          if (this.sec80DobjVal.parentAge === 'bellow60') {
            ...
            ).controls['hasParentOverSixty'].setValue(false);
          } else if (this.sec80DobjVal.parentAge === 'above60') {
            (
              (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
                'systemFlags'
              ] as FormGroup
            ).controls['hasParentOverSixty'].setValue(true);
          }
        }
      }
      ...
      if (this.businessFormValid) {
        ...
        if (
          ...
          this.utilsService.isNonEmpty(this.businessObject.tradeName44AD)
        ) {
          this.businessObject.natureOfBusiness44AD =
          ...
          if (
            this.utilsService.isNonEmpty(this.businessObject.recievedinCash)
          ) {
            let incomeObj = {
              id: null,
              incomeType: 'CASH',
        ...
        if (
          ...
          this.utilsService.isNonEmpty(this.businessObject.tradeName44ADA)
        ) {
          this.businessObject.natureOfBusiness44ADA =
        ...
        ).controls['presumptiveIncomes'].setValue(presumData);
      }

      /////////  taxPaid Bind part////////////////

      var blackObj = {};
      (this.itrSummaryForm.controls['assesse'] as FormGroup).controls['taxPaid']
        .value
        ? (this.itrSummaryForm.controls['assesse'] as FormGroup).controls[
            'taxPaid'
          ].value
      ...
      Object.assign(
        this.itrObject,
        this.itrSummaryForm.controls['assesse'].value
      );
  ...
  downloadItrSummary() {
    location.href =
      ...
      this.itrSummaryForm.value.summaryId;
  }
  ...
  deleteData(type: any, index: any) {
    if (type === 'Bank') {
      ...
      this.donationData
        ...
        .controls['donations'].setValue(this.donationData);
      this.getdeductionTotal(this.donationData);
  ...
  getBusinessData(businessInfo: any) {
    if (businessInfo.valid) {
      ...
      ).controls['financialParticulars'].patchValue(businessInfo.value);
  ...
  calIntersetFeess() {
    this.newRegimeTaxSummary.interestAndFeesPayable =
      Number(this.newRegimeTaxSummary.s234A) +
      Number(this.newRegimeTaxSummary.s234B) +
      Number(this.newRegimeTaxSummary.s234C) +
      Number(this.newRegimeTaxSummary.s234F);
    (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
      'interestAndFeesPayable'
    ].setValue(this.newRegimeTaxSummary.interestAndFeesPayable);

    this.newRegimeTaxSummary.agrigateLiability =
      Number(this.newRegimeTaxSummary.interestAndFeesPayable) +
      Number(this.newRegimeTaxSummary.totalTaxRelief);
  }
  ...
  showHouseBtn() {
    if (this.housingData.length > 0) {
      ...
      if (
        !this.newItrSumChanges ||
        this.housingData.length > 1 ||
        houseProType === 'LOP'
      ) {
        return true;
      } else {
  ...
  showTaxPaybleValue() {
    if (
      (this.itrSummaryForm.controls['assesse'] as FormGroup).controls['regime']
        .value === 'N'
    ) {
      if (
        (this.itrSummaryForm.controls['taxSummary'] as FormGroup).controls[
