...
export class LabFormComponent implements OnInit {
    ...
    if (this.data.mode === 'EDIT') {
      ...
      if (this.deductions instanceof Array && this.deductions.length > 0) {
        ...
        this.deductions.forEach(obj => {
          ...
          this.isDeductionsValid(this.immovableForm, this.deductions.indexOf(obj));
        });
        console.log('Immovable Form===', this.immovableForm);
  ...
  */
  ...
  improvementSelected() {
    ...
    return improve.controls.filter((item:FormGroup) => item.controls['selected'].value === true).length > 0;
  }
  ...
  */
  ...
  changeInvestmentSection(ref, index) {
    ...
    if (deductionForm.controls['underSection'].value === '54EE' || deductionForm.controls['underSection'].value === '54EC') {
      deductionForm.controls['costOfNewAssets'].setValidators([Validators.required, Validators.pattern(AppConstants.amountWithoutDecimal)]);
      ...
      if (ref === 'HTML') {
        ...
        deductionForm.controls['investmentInCGAccount'].updateValueAndValidity();
      }
    }
  ...
  calculateDeduction(index) {
    ...
    let request = {
      capitalGain: this.cgArrayElement?.assetDetails[this.currentCgIndex]?.capitalGain,
      capitalGainDeductions: [{
        ...
        cgasDepositedAmount: deductionForm.controls['investmentInCGAccount'].value,
        "saleValue": saleValue,
        "expenses": expenses
      }]};
    this.itrMsService.postMethod(param, request).subscribe((result: any) => {
      ...
      if(result?.success) {
        let finalResult = result.data.filter(item => item.deductionSection === `SECTION_${deductionForm.controls['underSection'].value}`)[0];
        deductionForm.controls['totalDeductionClaimed'].setValue(finalResult?.deductionAmount);
  ...
  investmentsCreateRowData() {
    ...
    this.deductions = this.cgArrayElement.deduction?.filter(deduction => (parseInt(deduction.srn) == this.cgArrayElement.assetDetails[this.currentCgIndex].srn));
    if(this.deductions)
  ...
  deleteInvestment(index) {
    ...
    const deductionsArray = <FormArray>this.immovableForm.get('deductions') as FormArray;
