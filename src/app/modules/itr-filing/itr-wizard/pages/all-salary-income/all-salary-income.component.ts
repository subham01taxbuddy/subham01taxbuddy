...
export class AllSalaryIncomeComponent extends WizardNavigation implements OnInit {
  ...
  deleteEmployers() {
    let employersArray = this.salaryIncomeFormGroup.controls['employersArray'] as FormArray;
    let nonSelected = employersArray.controls.filter((item: any) => item.controls['selected'].value === false);

    let employers = []
    nonSelected.forEach((employer: FormGroup) => {
      employers.push(
       this.Copy_ITR_JSON.employers.filter((item: any) => item.employerName == employer.controls['employerName'].value)[0]
      );
    })
  ...
  isEmployerSelected() {
    let employers = this.salaryIncomeFormGroup.controls['employersArray'] as FormArray;
    let selected = employers.controls.filter((item: any) => item.controls['selected'].value === true);
