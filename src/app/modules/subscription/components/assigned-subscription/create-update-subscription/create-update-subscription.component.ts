      ...
      this.utilsService.getUserCurrentStatus(this.userSubscription.userId).subscribe(
        (res: any) => {
          if (res.error) {
            ...
            if (this.selectedCouponCodeSubscriptionIds.length > 0 &&
                (this.userSubscription?.payableSubscriptionAmount < 0 ||
                this.userSubscription?.invoiceDetail?.some(invoice => invoice.paymentStatus === 'Paid'))) {
              ...
              this.loading = false;
              reject("Invalid coupon code application.");
              return;
            }

            if ((this.service === 'ITRU' || this.service === 'ITR') &&
                (!this.assessmentYear.value || typeof this.assessmentYear.value === 'undefined')) {
              this.loading = false;
              this.toastMessage.alert('error', 'Please select Financial Year For ' + (this.service === 'ITR' ? 'ITR' : 'ITR-U') + ' subscription');
              reject("Financial year not selected.");
              return;
            }

            if (this.showScheduledFields && (!this.scheduleCallService.value || typeof this.scheduleCallService.value === 'undefined')) {
              this.loading = false;
              this.toastMessage.alert('error', 'Please Select Service Type For Scheduled Call ');
              reject("Service type not selected.");
              return;
            }
            ...
            if (this.userSubscription.smeSelectedPlan != null && this.pin.value) {
              ...
              let requestData = JSON.parse(JSON.stringify(reqBody));
              this.itrService.postMethod(param, requestData).toPromise().then(
                (res: any) => {
                  this.loading = false;
              ...
              this.loading = false;
              reject("Plan & Pincode not selected.");
            }
          }
        },
        (error) => {
