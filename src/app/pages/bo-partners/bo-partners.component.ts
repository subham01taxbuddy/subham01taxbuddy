...
export class BoPartnersComponent implements OnInit {
  ...
  getBoPartners(){
    this.loading = true;
    let param = '/partner-details?page=0&size=10&from=2023-02-10&to=2023-02-13';
    this.userMsService.getMethod(param).subscribe((response: any) => {
       ...
       if (Array.isArray(response) && response.length > 0) {
        ...
        this.partnersGridOptions.api?.setRowData(this.createRowData(response));
      } else {
        this.loading = false;
  ...
  createRowData(data: any) {
    ...
    let boPartnersInfo: any = Object.assign({}, partnersArray[i], {
      ...
      currentstatus: data[i].currentstatus,
