...
export class DocumentUploadComponent implements OnInit {
  ...
  uploadDocument(type, document, password?){
    ...
    var s3ObjectUrl;
    if(type === 'PAN' || type === 'AADHAAR BACK' || type === 'AADHAAR FRONT'){
    ...
    let cloudFileMetaData = '{ "fileName":"'+document.name+'","userId":'+userId+',"accessRight":["'+userId+'_W"'+'],"origin":"BO", "s3ObjectUrl":"'+s3ObjectUrl+'","password":"'+(password ? password : "")+'"}';
    const formData = new FormData();
