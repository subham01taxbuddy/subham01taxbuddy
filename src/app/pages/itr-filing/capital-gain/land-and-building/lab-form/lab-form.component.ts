...
export class LabFormComponent implements OnInit {
  ...
  constructor(
    ...
    this.minSellDate = new Date(
      parseInt(this.ITR_JSON.assessmentYear.substring(0, 4)) - 1,
      3,
      1
    );
    this.maxSellDate = new Date(
      parseInt(this.ITR_JSON.assessmentYear.substring(0, 4)),
      2,
      31
    );
    ...
    return [
...

      {
        ...
        valueGetter: function nameFromCode(params) {
          return params.data.costOfNewAssets
            ? params.data.costOfNewAssets.toLocaleString('en-IN')
            : 0;
        },
      },

      {
        ...
        suppressMovable: true,
        valueGetter: function nameFromCode(params) {
          return params.data.investmentInCGAccount
            ? params.data.investmentInCGAccount.toLocaleString('en-IN')
            : 0;
        },
      },
      {
        ...
        suppressMovable: true,
        valueGetter: function nameFromCode(params) {
          return params.data.totalDeductionClaimed
            ? params.data.totalDeductionClaimed.toLocaleString('en-IN')
            : 0;
        },
      },
